<template>
    <div class="fade tab-pane" id="pills-bookmark"
        v-bind:class="(store.activeclass === 'pills_bookmark') ? 'active show' : ''" role="tabpanel"
...
function setFavourite(item) {
    const objIndex: number = favourite.findIndex(((obj) => obj.id === item.id));
    if (objIndex > -1) {
